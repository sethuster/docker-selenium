...
``` bash
$ docker run -d -p 4444:4444 -v /dev/shm:/dev/shm selenium/standalone-chrome:2.50.0
```
...
``` bash
$ docker run -d -p 4444:4444 selenium/standalone-chrome:2.50.0
# OR
$ docker run -d -p 4444:4444 selenium/standalone-firefox:2.50.0
```
...
``` bash
$ docker run -d -p 4444:4444 --name selenium-hub selenium/hub:2.50.0
```

### Chrome and Firefox Grid Nodes

``` bash
$ docker run -d --link selenium-hub:hub selenium/node-chrome:2.50.0
$ docker run -d --link selenium-hub:hub selenium/node-firefox:2.50.0
```
...
``` bash
$ docker run -d -p 4444:4444 -e JAVA_OPTS=-Xmx512m --name selenium-hub selenium/hub:2.50.0
```
...
``` bash
$ docker run -d -p 4444:4444 -e SE_OPTS=-debug --name selenium-hub selenium/hub:2.50.0
```
...
``` bash
$ docker run -d --name selenium-hub -p 4444:4444 selenium/hub:2.50.0
$ CH=$(docker run --rm --name=ch \
    --link selenium-hub:hub -v /e2e/uploads:/e2e/uploads \
    selenium/node-chrome:2.50.0)
```
...
``` bash
$ docker run -d -P --link selenium-hub:hub selenium/node-chrome-debug:2.50.0
$ docker run -d -P --link selenium-hub:hub selenium/node-firefox-debug:2.50.0
```

And for standalone: 
``` bash
$ docker run -d -p 4444:4444 selenium/standalone-chrome-debug:2.50.0
# OR
$ docker run -d -p 4444:4444 selenium/standalone-firefox-debug:2.50.0
```
...
``` dockerfile
#FROM selenium/node-chrome-debug:2.50.0
#FROM selenium/node-firefox-debug:2.50.0
...
selenium/node-firefox           2.50.0              69f762d0d79e        29 minutes ago      552.1 MB
selenium/node-chrome            2.50.0              9dd73160660b        30 minutes ago      723.6 MB
selenium/node-base              2.50.0              1b7a0b7024b1        32 minutes ago      426.1 MB
selenium/hub                    2.50.0              2570bbb98229        33 minutes ago      394.4 MB
selenium/base                   2.50.0              33478d455dab        33 minutes ago      362.6 MB
